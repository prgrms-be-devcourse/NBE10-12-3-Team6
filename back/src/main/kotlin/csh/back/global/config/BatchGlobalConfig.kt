...
class BatchGlobalConfig {
    ...
    fun globalBatchTaskExecutor(): ThreadPoolTaskExecutor {
        val executor = ThreadPoolTaskExecutor()
        executor.corePoolSize = 10 // 기본 대기 스레드 수
        executor.maxPoolSize = 50 // 최대 확장 스레드 수
    ...
    // ---------------------------------------------------------
    @Bean
    fun globalJobListener(): JobExecutionListener {
        return object : JobExecutionListener {
            ...
            override fun afterJob(jobExecution: JobExecution) {
                val duration = Duration.between(jobExecution.startTime, jobExecution.endTime).toMillis()
                if (jobExecution.status == BatchStatus.FAILED) {
                    log.error(
                        " [배치 실패] Job Name: {}, 걸린 시간: {}ms",
                        jobExecution.jobInstance.jobName, duration
                    )
                    //  실무에서는 보통 여기에 Slack/Teams API 호출 코드를 넣어서
                    //  배치가 터지면 개발팀 메신저로 알람이 오게 만듦
                } else if (jobExecution.status == BatchStatus.COMPLETED) {
                    log.info(
                        " [배치 성공] Job Name: {}, 걸린 시간: {}ms",
                        jobExecution.jobInstance.jobName, duration
                    )
                }
            }
    ...
    // ---------------------------------------------------------
    @Bean
    fun globalRunIdIncrementer(): RunIdIncrementer {
